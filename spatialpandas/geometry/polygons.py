...
from .base import Geometry, GeometryDtype, GeometryArray, _geometry_map, ngjit
from .lines import compute_length
...
class PolygonsDtype(GeometryDtype):
    _type_name = "Polygons"
...
class PolygonsArray(GeometryArray):
    ...
    def length(self):
        ...
        _geometry_map(self.start_indices, self.flat_array, result, compute_length)
        return result

    @property
    def area(self):
        result = np.zeros(self.start_indices.shape, dtype=self.flat_array.dtype)
        _geometry_map(self.start_indices, self.flat_array, result, compute_area)
        return result
